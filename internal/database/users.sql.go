...
const upgradeUserPolkaByID = `-- name: UpgradeUserPolkaByID :exec
UPDATE users SET is_chirpy_red = TRUE, updated_at = NOW()
WHERE id = $1
`

func (q *Queries) UpgradeUserPolkaByID(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.ExecContext(ctx, upgradeUserPolkaByID, id)
	return err
}
